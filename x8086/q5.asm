; Computer Language and Structure, HW3, Q5
...
Start:  MOV     AX,DtSeg    ; set DS to point to the data segment
        ...
        LEA     DX,prompt   ; load prompt message in DX
        MOV     AH,9        ; set INT 21H code to print string
        INT     21H         ; call INT 21H
        
        PUSH    0           ; push 0 as placeholder of return value
        CALL    readI       ; call readI to read integer
        POP     DX          ; pop return value in DX
        
        MOV     CX,0        ; set CX = 0 - counter initialization 
        
cLoop:  CMP     CX,DX       ; compare CX with DX
        JG      cLoopE      ; loop condition : CX <= DX
        PUSH    0           ; push 0 as place holder for return value
        PUSH    CX          ; push CX as arg for calc
        CALL    calc        ; call calc
        POP     CX          ; clear up stack
        CALL    pEndl       ; next line
        CALL    printUI     ; print return arg
        POP     AX          ; pop return arg
        INC     CX          ; increament CX
        JMP     cLoop       ; go back to loop condition
cLoopE:
...
; calc: subroutine to calculate f(n)=2^n + n
; Instruction for stack
; PUSH place holder of return value
; PUSH n
calc    PROC    NEAR        ; calca subrotine to calculate 
        ...
        MOV     CX,[BP+10]  ; set CX equal to arg, we will use CL beacause CX < 16
        MOV     AX,1        ; set AX = 2^0
        SHL     AX,CL       ; shift AX to left by CL (AX = 2^CL)
        ADD     AX,[BP+10]  ; AX = AX + arg
        MOV     [BP+12],AX  ; set return value equal to AX
